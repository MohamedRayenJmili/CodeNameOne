...
public class statForm extends Form {
    ...
    public statForm(Form previous) {
        ...
        List<Evenement> offres = ServiceEvenement.getInstance().affichageEvenement();
        Map<String, Integer> countByCategory = new HashMap<>();
for (Evenement offre : offres) {
    int category = offre.getNbMax();
    if (countByCategory.containsKey(offre.getTitreEv())) {
        countByCategory.put(offre.getTitreEv(), countByCategory.get(offre.getTitreEv()) + category);
    } else {
        countByCategory.put(offre.getTitreEv(), category);
    }
}
